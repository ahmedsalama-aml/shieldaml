"""
ShieldAML — Main Server (Pure Python — no pydantic, no FastAPI)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/health":
            return self.send_json({"status":"online","system":"ShieldAML","version":"1.0.0","compliance":"FATF 2023 · FRA Law 161/2024"})
        ...
        if path == "/api/docs":
            return self.send_json({"ShieldAML API":"v1.0.0","note":"POST /api/transactions/analyze to analyze a transaction"})
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/str/generate":
                ...
                return self.send_json({"success":True,"str_id":create_str_report(body.get("transaction_id"), analysis, tx)})
...
if __name__ == "__main__":
    ...
    init_db()
    print(f"✅ ShieldAML started on port {port}")
